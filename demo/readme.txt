...
(done) With the joystick it is possible to draw on the OLED display
With the Rotary switch it is possible to control the 7-segment display
With the trimpot it is possible to control the RGB LED
(done) Moving the board (accelerometer) will control a running pattern on the LEDs
(done) Click SW3 button to start playing a melody on the Speaker
